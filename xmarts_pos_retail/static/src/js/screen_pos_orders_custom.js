"use strict";
odoo.define('xmarts_pos_retail.screen_pos_orders_custom', function (require) {
    ...
    models.load_fields('pos.order', ['laundry_state','laundry_priority']);
    ...
    var pos_orders_screen = screens.ScreenWidget.extend({
        ...
        render_pos_order_list: function (orders) {
            ...
            for (var i = 0, len = Math.min(orders.length, 1000); i < len; i++) {
                ...
                this.set_extra_laundry_info(order);
        ...
        hide_order_selected: function () { // hide when re-print receipt
         ...
         set_extra_laundry_info: function(order){  
            var self = this;  
            rpc.query({
                model: 'pos.order',
                method: 'compute_laundry',
                args:[[order['id']]],
            }).then(function (r) {
                var res = r.r;
                var s = r.s;
        ...
        display_pos_order_detail: function (order) {
            ...
            this.set_extra_laundry_info(this.order_selected);
            ...
            this.$('.create_invoice').click(function () {
               ...
               this.$('.procesa_orden').click(function () {
                ...
                rpc.query({
                model: 'pos.order',
                method: 'compute_laundry_state',
                args:[[order['id'],'proceso']],
                }).then(function (res) {
                    var res = res.res;
                    order['laundry_state'] = res;
                    self.display_pos_order_detail(order);
            ...
            this.$('.termina_orden').click(function () {
                var order = self.order_selected;
                rpc.query({
                model: 'pos.order',
                method: 'compute_laundry_state',
                args:[[order['id'],'terminado']],
                }).then(function (res) {
                    var res = res.res;
                    order['laundry_state'] = res;
                    self.display_pos_order_detail(order);
            ...
            this.$('.entrega_orden').click(function () {
                var order = self.order_selected;
                rpc.query({
                model: 'pos.order',
                method: 'compute_laundry_state',
                args:[[order['id'],'entregado']],
                }).then(function (res) {
                    var res = res.res;
                    order['laundry_state'] = res;
                    self.display_pos_order_detail(order);
